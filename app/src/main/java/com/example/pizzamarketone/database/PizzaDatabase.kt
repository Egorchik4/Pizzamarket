...
object PizzaDatabase {
    ...
    // лямбда функция для перебора элементов List (mapIndexed - для возможности перебора по индексам) нумерация не с "0", а с "1" !!!
    private val pizzaList: List<PizzaEntity> = pizzaSet.mapIndexed { index, it ->
        PizzaEntity(
            index + 1,
            it.name,
            it.price,
